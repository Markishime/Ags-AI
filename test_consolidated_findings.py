...
for finding in findings:
    if 'Critical Soil Health Analysis' in finding.get('title', '') or 'Leaf Nutrient Deficiency Analysis' in finding.get('title', ''):
        print(f"\n{finding['title']}:")
        print(finding['description'][:1000] + ('...' if len(finding['description']) > 1000 else ''))
